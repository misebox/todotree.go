...
func TestEmbed(t *testing.T) {
	...
	if !reflect.DeepEqual(want, rawPubKey[:len(want)]) {
		t.Errorf("want %s, but got %s", want, rawPubKey)
	}
	want = []byte("-----BEGIN RSA PRIVATE KEY-----")
	if !reflect.DeepEqual(want, rawPrvKey[:len(want)]) {
		t.Errorf("want %s, but got %s", want, rawPrvKey)
	}
}
...
func TestJWTer_GetToken(t *testing.T) {
	...
	prvKey, err := jwk.ParseKey(rawPrvKey, jwk.WithPEM(true))
	if err != nil {
		t.Fatal(err)
	}
	signed, err := jwt.Sign(want, jwt.WithKey(jwa.RS256, prvKey))
	if err != nil {
		t.Fatal(err)
	}
...
func TestJWTer_GetToken_NG(t *testing.T) {
	...
	signed, err := jwt.Sign(tok, jwt.WithKey(jwa.RS256, prvKey))
	if err != nil {
		t.Fatal(err)
	}
